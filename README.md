...
current version: [v0.8.4](https://github.com/timfayz/pretty/tags)
...
$ zig fetch --save https://github.com/timfayz/pretty/archive/v0.8.4.tar.gz
$ cat build.zig.zon
..
  .pretty = .{
      .url = "https://github.com/timfayz/pretty/archive/v0.8.4.tar.gz",
      .hash = "..long_hash..",
  },
..
```
...
1. `pretty.dump` – generates a string and returns it as a `[]u8` slice.
1. `pretty.dumpAsList` – generates a string and returns it as `std.ArrayList` interface.
...
use options to query or "cut off" unnecessary output:
